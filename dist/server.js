...
app.post("/items/suggested", async (req, res) => {
    try {
        ...
        const clicks = await ProductClick_1.default.find(clickQuery).sort({ clickedAt: -1 }).limit(4).select("productId").lean();
        if (!clicks.length) {
            return res.json({ products: [], reason: "no-clicks" });
        }
        const clickedIds = [...new Set(clicks.map(c => c.productId.toString()))];
        const products = await Item_1.default.find({ _id: { $in: clickedIds } }).sort({ _id: -1 }).limit(4).select("itemId name price actualPrice image category fabric size").lean();
